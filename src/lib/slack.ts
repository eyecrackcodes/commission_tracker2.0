...
export async function sendPolicyNotification(
  data: PolicyNotificationData
): Promise<boolean> {
  try {
    if (!process.env.SLACK_BOT_TOKEN || !process.env.SLACK_CHANNEL_ID) {
      console.error(
        "Slack configuration missing. Please set SLACK_BOT_TOKEN and SLACK_CHANNEL_ID in your environment variables."
      );
      return false;
    }
...
export async function sendQuickPolicyPost(
  carrier: string,
  product: string,
  premium: number,
  acronym: string = "OCC",
  ...
  try {
    ...
    const text = `${acronym} - ${carrier} | ${product} | ${formattedPremium}${agentInfo}`;
